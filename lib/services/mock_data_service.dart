...
import '../data/api/subject_api_service.dart';
import '../data/api/test_api_service.dart';
import '../data/api/question_api_service.dart';
...
class MockDataService {
  ...
  Future<List<QuestionModel>> getQuestionsForTest(String testId) async {
    try {
      ...
      // Fallback to mock data if API fails
      return _getMockQuestionsForTest(testId);
    }
  }

  // Get mock questions for a test
  Future<List<QuestionModel>> _getMockQuestionsForTest(String testId) async {
    ...
    final dailyQuiz = await getDailyQuiz();
    int questionCount = 20;
    if (testId == dailyQuiz['id']) {
      questionCount = dailyQuiz['total_questions'] ?? 20;
    } else if (test != null) {
  ...
  // Get Daily Quiz
  // Returns daily quiz from API or fallback mock data
  Future<Map<String, dynamic>> getDailyQuiz() async {
    try {
      final dailyQuiz = await _testApiService.getDailyQuiz();
      if (dailyQuiz != null) {
        // Return the API response directly - it matches the backend format
        return dailyQuiz;
      }
    } catch (e) {
    ...
    return {
      'id': 'daily_quiz_fallback',
      'title': 'Daily Quiz Challenge2',
      'description': 'Quick practice test for revision',
      'subject_id': 'polity',
      'chapter_id': null,
      'test_type': 'daily',
      'total_questions': 20,
      ...
      'is_timed': true,
      'instructions': 'Quick 30-minute practice test. Focus on accuracy.',
      'is_published': true,
      'allow_review': true,
