...
class PolityContentService {
  ...
  Future<List<ChapterModel>> getPolityChapters() async {
    try {
      final chaptersData = await _subjectApiService.getSubjectChapters('polity'); //'/subjects/$subjectId/chapters'
      return chaptersData
  ...
  Future<List<TestModel>> getChapterTests() async {
    try {
      ...
      final testsData = await _testApiService.getTests(subjectId: 'polity');  // /tests
      return testsData
      ...
      return chapters.map((chapter) {
        return TestModel(
          id: 'test_${chapter.id}',
          subjectId: 'polity',
          chapterId: chapter.id, // Set chapter_id for chapter tests
  ...
  TestModel getFullLengthTest() {
    return TestModel(
      id: 'polity_full_test',
      subjectId: 'polity',
      chapterId: null, // Full-length tests don't have chapter_id
